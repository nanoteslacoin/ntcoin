...
public:
    ...
    void updateWallet(const uint256 &hash, int status)
    {
        OutputDebugStringF("updateWallet %s %i\n", hash.ToString().c_str(), status);
        {
            ...
            if(status == CT_UPDAnTcoin)
            {
                if(showTransaction && !inModel)
                    status = CT_NEW; /* Not in model, but want to show, treat as new */
                if(!showTransaction && inModel)
                    status = CT_DELEnTcoin; /* In model, but want to hide, treat as deleted */
            }
            ...
            case CT_DELEnTcoin:
                if(!inModel)
                {
                    OutputDebugStringF("Warning: updateWallet: Got CT_DELEnTcoin, but transaction is not in model\n");
                    break;
                }
